...
# 3. Navigation (Ajout de "L'épreuve")
...
if menu == "Home":
    ...
    # Affichage du logo (Lien vers ton image de points colorés sur GitHub)
    logo_url = "https://raw.githubusercontent.com/ccarton51-cloud/GO-app/main/logo.png" 
...
# --- AUTRES PAGES (DYNAMIQUES) ---
    ...
    try:
            ...
            if liens_valides:
                if len(liens_valides) > 1:
                    ...
                    for idx, l in enumerate(liens_valides):
                        st_cols[idx].image(l, use_container_width=True)
                else:
                    st.image(liens_valides[0], use_container_width=True)

            # Vidéo / Conseils / Textes additionnels
        ...
        st.info("Sélectionnez une section dans le menu ou vérifiez le nom de l'onglet Google Sheet.")
